...
static void vPeriodicTask(void *pvParameters) {
    const TickType_t xDelay250ms = pdMS_TO_TICKS(500UL); // Shorter delay
...
static void vHandlerTask(void *pvParameters) {
    const TickType_t xMaxExpectedBlockTime = xInterruptFrequency + pdMS_TO_TICKS( 100 );
 // Longer delay
...
static uint32_t ulExampleInterruptHandler(void) {
    ...
    /* Request a context switch if the handler task has higher priority */
